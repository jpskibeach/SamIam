...
//{superfluous} import il2.util.IntSet;
//{superfluous} import il2.model.Index;
...
public class DgraphCreateHMetis1 extends DgraphCreateHMetis {
	...
	public DgraphCreateHMetis1( int[] options, int ubfactor, int numGlobalTrials, int numLocalTrials) {
		super( options, ubfactor, numGlobalTrials, numLocalTrials);
	}

	public DgraphCreateHMetis1( int ubfactor, int numGlobalTrials, int numLocalTrials) {
	...
	public DGraph create( Collection data) {
		...
		DGraph dg;
		{
			...
			createSubTree( hg, leafNodes, leafNodesToInts, tree);
		...
		for( int i=1; i<numGlobalTrials; i++) {
			...
			createSubTree( hg, leafNodes, leafNodesToInts, tmp_tr);
    ...
    private Integer createSubTree( Hypergraph hg, ArrayList leafNodes, Map leafNodesToInts, Graph tree) {
        if( hg.numNodes() == 1) {
